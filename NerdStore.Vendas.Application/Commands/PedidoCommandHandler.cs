      ...
      IRequestHandler<CancelarProcessamentoPedidoCommand, bool>
{
    ...
    public async Task<bool> Handle(AdicionarItemPedidoCommand message, CancellationToken cancellationToken)
    {
        ...
        if (pedido == null)
        {
            var pedidoNovo = Pedido.PedidoFactory.NovoPedidoRascunho(message.ClienteId);
            pedidoNovo.AdicionarItem(pedidoItem);

            _pedidoRepository.Adicionar(pedidoNovo);
        ...
        else
        {
            ...
            {
                _pedidoRepository.AdicionarItem(pedido.PedidoItems.FirstOrDefault(p => p.ProdutoId == pedidoItem.ProdutoId));
            }
            else
            {
    ...
    public async Task<bool> Handle(RemoverItemPedidoCommand message, CancellationToken cancellationToken)
    {
        ...
        pedido.AdicionarEvento(new PedidoAtualizadoEvent(message.ClienteId, pedido.Id, pedido.ValorTotal));
        pedido.AdicionarEvento(new PedidoProdutoRemovidoEvent(message.ClienteId, message.ProdutoId, pedido.Id));
